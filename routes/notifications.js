...
router.get("/todaynotifs/:id", authorization, (request, response) => {
  ...
  database
    .query(
      "SELECT * FROM scoop.notifications LEFT JOIN (SELECT scoop.users.firstname AS activityfirstname, scoop.users.lastname AS activitylastname, scoop.users.userid AS activityuserid, \
...
router.get("/todayimages/:id", authorization, (request, response) => {
  ...
  database
    .query(
      "SELECT t1.activityprofileimage, t2.likesprofileimage FROM scoop.notifications LEFT JOIN (SELECT scoop.users.profileimage AS activityprofileimage, scoop.postcomment.activityid AS activityactivityid FROM \
        scoop.postcomment INNER JOIN scoop.users ON \
        scoop.users.userid = scoop.postcomment.userid WHERE scoop.postcomment.activestatus = 1) t1 ON scoop.notifications.activityid = t1.activityactivityid \
    ...
    .then(results => {
      for (i = 0; i < results.length; i++) {
        if (results[i].activityprofileimage != null) {
          //checks if activity profile image is not null
        ...
        } else if (results[i].likesprofileimage != null) {
          //checks if likes profile image is not null
          ...
          //otherwise all the images are null
 ...
 */
router.get("/recentnotifs/:id", authorization, (request, response) => {
  const userid = request.params.id; //gets the user id passed in
